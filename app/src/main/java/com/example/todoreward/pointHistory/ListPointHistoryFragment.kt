package com.example.todoreward.pointHistory
...
import com.example.todoreward.SpacingItemDecoration
import com.example.todoreward.reward.ItemReward
import com.example.todoreward.reward.RewardItemModel
import com.example.todoreward.task.ItemToDo
import com.example.todoreward.task.TodoItemModel
import com.example.todoreward.toast
import java.util.*
...
class ListPointHistoryFragment : Fragment()   {
    ...
    private fun addHistoryPoint(toDo: ItemToDo) {
        ...
        val newHistoryPoint: ItemHistoryPoint =
            ItemHistoryPoint.createHistoryPointForTask(
                toDo.taskName, toDo.points,
                dateStr
            )
    ...
    private fun addHistoryPoint(reward: ItemReward) {
        ...
        val dateStr = "${date.get(Calendar.MONTH)}/${date.get(Calendar.DAY_OF_MONTH)}/${date.get(Calendar.YEAR)}"
        val newHistoryPoint: ItemHistoryPoint =
            ItemHistoryPoint.createHistoryPointForReward(
                reward.rewardName, reward.pointCost,
                dateStr
            )
        addHistoryPoint(newHistoryPoint)
    }
